...
const UpdateCampaign = () => {
  ...
  const initialImageUrls = Array.isArray(state.image)
    ? state.image.join("\n")
    : state.image || "";
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const image = parseImageUrls(form.imageUrls);

    setIsLoading(true);
    try {
      const { imageUrls, ...campaignData } = form;
      ...
      alert(error?.reason || error?.message || "Campaign update failed.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className=" flex justify-center items-center flex-col rounded-[10px] sm:p-10 mt-8 p-4 bg-white dark:bg-[#1c1c24]">
      ...
      >
        ...
        <FormField
          labelName="Campaign image URLs (optional)"
          placeholder="Paste image URLs (comma or new line separated)"
          isTextArea
          value={form.imageUrls}
          handleChange={(e) => handleFormFieldChange("imageUrls", e)}
        />
